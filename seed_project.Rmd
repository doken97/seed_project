...
#Changing the weather set to reflect the target farm
weather_set[,c("Latitude","Longitude","Soil_Type","PH1","Clay1","Sand1","CEC")] = Seed.Eval[,c("Latitude","Longitude","Soil_Type","PH1","Clay1","Sand1","CEC")]

#Sample 1000 rows of weather set with replacement
weather_set = weather_set[sample(nrow(a), 1000,replace = TRUE),]

##First the sufficient data sets--> varieties turn numeric
tree.PredictSuf = list()
for (i in 1:length(unique(newdata$Variety))){
  if(length(SplitData[[i]]$Variety !=0)){
    a = weather_set
    b = SplitData[[i]]
    c = rbind(a,b)
    c$Soil_Type = as.factor(c$Soil_Type)
    c = c[-c(1:dim(a)[1]),]
    c = c[sample(nrow(c), 1000,replace = TRUE),]
    c[c("Temp_03","Temp_04","Temp_05","Temp_06","Temp_07","Temp_08","Temp_09","Median_Temp","Prec_03","Prec_04","Prec_05","Prec_06","Prec_07","Prec_08","Prec_09","Median_Prec","Rad_03","Rad_04","Rad_05","Rad_06","Rad_07","Rad_08","Rad_09","Median_Rad")] = a[c("Temp_03","Temp_04","Temp_05","Temp_06","Temp_07","Temp_08","Temp_09","Median_Temp","Prec_03","Prec_04","Prec_05","Prec_06","Prec_07","Prec_08","Prec_09","Median_Prec","Rad_03","Rad_04","Rad_05","Rad_06","Rad_07","Rad_08","Rad_09","Median_Rad")]
    tree.Variety= tree(as.formula(paste(target,"~.")),SplitData[[i]][,c(predictors.treeSuf,target)])
    set.seed(1)
    ...
    tree.PredictSuf[[i]]=predict(pruned.Tree,newdata=c)
  }
}
...
tree.Sufvar = c()
for (i in 1:83){
  if (length(tree.PredictSuf[[i]]) != 0){
    tree.Sufmeans[i] = mean(tree.PredictSuf[[i]])
    tree.Sufvar[i] = var(tree.PredictSuf[[i]])
  }
}

order(tree.Sufmeans, decreasing = TRUE)[1:40]
sort(tree.Sufmeans,decreasing = TRUE)[1:40]
order(tree.Sufvar, decreasing = FALSE)[1:40]
sort(tree.Sufvar,decreasing = FALSE)[1:40]

tree.PredictInsuf = list()
for(i in 1:length(InsufVariety.vector)){
  a = weather_set
  b = InsufVariety.df[which(as.numeric(InsufVariety.df$Variety) == InsufVariety.vector[i]),]
  c = rbind(a,b)
  ...
  c = c[sample(nrow(c), 1000,replace = TRUE),]
  c[c("Temp_03","Temp_04","Temp_05","Temp_06","Temp_07","Temp_08","Temp_09","Median_Temp","Prec_03","Prec_04","Prec_05","Prec_06","Prec_07","Prec_08","Prec_09","Median_Prec","Rad_03","Rad_04","Rad_05","Rad_06","Rad_07","Rad_08","Rad_09","Median_Rad")] = a[c("Temp_03","Temp_04","Temp_05","Temp_06","Temp_07","Temp_08","Temp_09","Median_Temp","Prec_03","Prec_04","Prec_05","Prec_06","Prec_07","Prec_08","Prec_09","Median_Prec","Rad_03","Rad_04","Rad_05","Rad_06","Rad_07","Rad_08","Rad_09","Median_Rad")]
  tree.Variety = tree(as.formula(paste(target,"~.")),InsufVariety.df[,c(predictors.treeSuf,target)])
  set.seed(1)
  crossV.Tree =cv.tree(tree.Variety,FUN=prune.tree)
  best.size = crossV.Tree$size[which(crossV.Tree$dev == min(crossV.Tree$dev))]
  pruned.Tree = prune.tree(tree.Variety,best = max(best.size,2))
  tree.PredictInsuf[[i]]=predict(pruned.Tree,newdata=c)
}

tree.Insufmeans = c()
tree.Insufvar = c()
for (i in 1:99){
  if (length(tree.PredictInsuf[[i]]) != 0){
    tree.Insufmeans[i] = mean(tree.PredictInsuf[[i]])
    tree.Insufvar[i] = var(tree.PredictInsuf[[i]])
  }
}

sort(tree.Insufmeans, decreasing = TRUE)
order(tree.Insufmeans, decreasing = TRUE)
sort(tree.Insufvar, decreasing = FALSE)[-(1:32)]
order(tree.Insufvar,decreasing = FALSE)[-(1:32)]
#Seed Winners (omitting 93 due to higher variance)
#Heuristic Method
#58 (61.97, 11.45), 60(61.97,11.45), 93(61.8, 14.86), 35(60.9, 10.25), 61(60.90,10.25)

unique(newdata$Variety[newdata$Variety%in%SufVariety.df$Variety == FALSE])[c(58,60,35,61)]
##Recomendation: V206, V142, V28, and V146 are the winners!
```